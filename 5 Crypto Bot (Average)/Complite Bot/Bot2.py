...
transaction = ''
prev_avg = 0
with open("Bot2.csv", 'w', newline='') as file:
      writer = csv.writer(file)
      writer.writerow(["SN","DateTime", "Transaction", "Transaction Price", "Closing Price", "Average", "Investment", "Portfolio", "Money end", "Real Time Portfolio Vlaue"])
...
# Bitcoin Bot
def on_close(ws):
	print("we have to settle all the payment here")
...
def on_message(ws,message):
  ...
  if candle_closed:
    ...
    if len(closes) == 1:
    	buy(50, closes[-1])
    	sell_pur_hist.append(["Buy", 50, closes[-1]])
    	stock_avg_price = closes[-1]
    	a.append(50/closes[-1]) # a = 5
    	...
    	if stock_avg_price > closes[-1] or stock_avg_price == 0 :
            if money_end > 0 :
                ...
                # transaction = "Buy"
                sell_pur_hist.append(["Buy", 50, closes[-1]])
                a.append(50/closes[-1]) # number of stock buy
                b.append(closes[-1])
                c.append(a[-1]*b[-1])
    ...
    with open("Bot2.csv", 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([len(closes),utc_date, transaction, transaction_price, closes[-1], prev_avg, investment[-1], portfolio, money_end, RT_portfolio_value]) # "Investment", "Portfolio", "Money end", "Real Time Portfolio Vlaue"
